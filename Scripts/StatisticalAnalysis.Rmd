...
md_sub_n <- md_sub_n %>% filter(NitrateN > 0)
min(md_sub_n$NitrateN)

#check residauls
mod <- aov(log(NitrateN) ~ Covercrop + Timepoint + WheatM, data = md_sub_n)
shapiro_test(mod$residuals)
hist(mod$residuals)


#xplore data
boxplot(log(md_sub_n$NitrateN) ~ md_sub_n$WheatM)
boxplot(log(md_sub_n$NitrateN) ~ md_sub_n$Covercrop)
boxplot(md_sub_n$NitrateN ~ md_sub_n$Covercrop)
boxplot(log(md_sub_n$NitrateN) ~ md_sub_n$Timepoint)


res_aov <- md_sub_n %>% anova_test(log(NitrateN) ~ Covercrop + Timepoint + WheatM)
print(res_aov)


      
res_tukey <- md_sub_n %>% tukey_hsd(formula = log(NitrateN) ~ Covercrop + Timepoint + WheatM)
print(res_tukey[res_tukey$p.adj<0.06,])
...
```{r}
#check residauls
mod <- aov(log(AmmoniumN) ~ Covercrop + Timepoint + WheatM, data = md)
shapiro_test(mod$residuals)
hist(mod$residuals)


#xplore data
boxplot(log(md$AmmoniumN) ~ md$WheatM)
boxplot(md$AmmoniumN ~ md$Covercrop)
boxplot(md$AmmoniumN ~ md$Timepoint)


res_aov <- md %>% anova_test(log(AmmoniumN) ~ Covercrop + Timepoint + WheatM)
print(res_aov)

      
res_tukey <- md %>% tukey_hsd(formula = log(AmmoniumN) ~ Covercrop + Timepoint + WheatM)
print(res_tukey[res_tukey$p.adj<0.06,])
...
#global
#ph
mod<- aov(log(NitrateN) ~ Covercrop + WheatM, data = md)
shapiro_test(mod$residuals)
hist(mod$residuals)
boxplot(md$NitrateN ~ md$WheatM)
boxplot(md$NitrateN ~ md$Covercrop)
#boxplot(md$pH ~ md$Timepoint)
...
res_aov <- md %>% anova_test(AmmoniumN ~ Covercrop  + WheatM)
print(res_aov)

res_aov <- md %>% anova_test(log(NitrateN) ~ Covercrop  + WheatM)
print(res_aov)


      
res_tukey <- md %>% tukey_hsd(formula = log(NitrateN) ~ Covercrop + WheatM)
print(res_tukey[res_tukey$p.adj<0.06,])
...
#just ran and swtiched the response variable instaed of recoding or loopoing. I'm being lazy and want to get this done.
#checked res noramlity by chaing respone 
md<-data.frame(sample_data(subset_samples(ps16S_bac_rarefy_iter, Treatment == "CC+WM")))
#global
#ph
mod<- aov(NitrateN ~ Timepoint, data = md)
shapiro_test(mod$residuals)
hist(mod$residuals)
boxplot(md$NitrateN ~ md$Timepoint)
...
res_aov <- md %>% anova_test(AmmoniumN ~ Timepoint)
print(res_aov)

res_aov <- md %>% anova_test(NitrateN ~ Timepoint)
print(res_aov)
      
res_tukey <- md %>% tukey_hsd(formula = NitrateN ~ Timepoint)
print(res_tukey[res_tukey$p.adj<0.06,])
```
...
#      d = "Covercrop"
 ...
 # sd$Timepoint <- factor(sd$Timepoint, levels = c("to", "t1", "t2", "t3", "t4", "t5", "t6"))
  ...
  wu.sd_test_plotting <- wu.sd_test %>% select(Var1, Var2, value, Type1, WheatM.x, time.x, Type2, Timeval1, Timeval2)
