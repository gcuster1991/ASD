...
update metdata
```{r}
meta_w_eh <- read.csv("/Users/gordoncuster/Desktop/Git_projects/ASD/Data/ASD_MetaWEh.csv")
rownames(meta_w_eh) <- meta_w_eh$sample.names
md_w_eh<-sample_data(meta_w_eh)

sample_names(ps16S_bac_rarefy_iter)
sample_data(ps16S_bac_rarefy_iter) <- meta_w_eh

sample_data(ps16S_bac_rarefy_iter)
```
...
ps16S_bac_rarefy_iter_noT0 <- subset_samples(ps16S_bac_rarefy_iter, Timepoint != "t0")
...
res_aov <- md %>% anova_test(pH ~ Covercrop *  WheatM + Timepoint )
print(res_aov)
...
res_aov <- md_sub %>% anova_test(EC ~ Covercrop *  WheatM + Timepoint)
print(res_aov)
...
#md_sub_n <- md_sub %>% filter(NitrateNperkgDrySoil > 0)
md_sub_n <- md_sub %>% mutate(NitrateNperkgDrySoil = NitrateNperkgDrySoil + 0.00001)
...
res_aov <- md_sub_n %>% anova_test(log(NitrateNperkgDrySoil) ~ Covercrop *  WheatM + Timepoint)
print(res_aov)
...
res_aov <- md %>% anova_test(log(NH4NperkgDrySoil) ~ Covercrop *  WheatM + Timepoint)
print(res_aov)
...
Aov of Eh and cumulative Eh

```{r}
md<-data.frame(sample_data(ps16S_bac_rarefy_iter_noT0 ))
md$CumulativeEh

mod <- aov(log(CumulativeEh+0.1) ~ Covercrop + Timepoint + WheatM, data = md)
shapiro_test(mod$residuals)
hist(mod$residuals)


#xplore data
boxplot(md$CumulativeEh ~ md$WheatM)
boxplot(md$CumulativeEh ~ md$Covercrop)
boxplot(md$CumulativeEh ~ md$Timepoint)


res_aov <- md %>% anova_test(log(CumulativeEh+0.1) ~ Covercrop *  WheatM + Timepoint)
print(res_aov)

      
res_tukey <- md %>% tukey_hsd(formula = log(CumulativeEh+0.1) ~ Covercrop + Timepoint + WheatM)
print(res_tukey[res_tukey$p.adj<0.06,])
```
...
mod <- aov(AverageEh ~ Covercrop + Timepoint + WheatM, data = md)
shapiro_test(mod$residuals)
hist(mod$residuals)
...
#xplore data
boxplot(md$AverageEh ~ md$WheatM)
boxplot(md$AverageEh ~ md$Covercrop)
boxplot(md$AverageEh ~ md$Timepoint)


res_aov <- md %>% anova_test(AverageEh ~ Covercrop *  WheatM + Timepoint)
print(res_aov)

      
res_tukey <- md %>% tukey_hsd(formula = AverageEh ~ Covercrop + Timepoint + WheatM)
print(res_tukey[res_tukey$p.adj<0.06,])
```
...
calculate summary stats in table
```{r}
md<-data.frame(sample_data(ps16S_bac_rarefy_iter_noT0))

#md %>% filter(Timepoint == "t0") %>%
 summ_phyico <- md %>%
  group_by(Timepoint, Treatment) %>%
 summarize(
    across(c(pH, EC, NitrateNperkgDrySoil, NH4NperkgDrySoil, AverageEh, CumulativeEh), list(mean = ~ round(mean(.x, na.rm = TRUE), 3), sd = ~ round(sd(.x, na.rm = TRUE), 3)), .names = "{.col}_{.fn}")
  )
...
summ_alpha<- alphadiv %>%
 ...
 summarize(
    across(c(Observed, Shannon, Chao1), list(mean = ~ round(mean(.x, na.rm = TRUE), 3), sd = ~ round(sd(.x, na.rm = TRUE), 3)), .names = "{.col}_{.fn}")
  )

plot(alphadiv$Shannon ~ alphadiv$wheatm)

alphadiv$treatment <- factor(alphadiv$treatment, levels = c("F+W", "F+WM", "CC", "CC+WM", "Tr", "Tr+WM", "2spp", "2spp+WM"), ordered = T)
...
ggplot(alphadiv) + geom_boxplot(aes(x = time, y = Observed, color = treatment, group = time)) + facet_wrap("treatment", nrow = 2)
ggplot(alphadiv) + geom_boxplot(aes(x = treatment, y = Observed, color = treatment)) + facet_wrap("time", nrow = 2)

ggplot(alphadiv) + geom_boxplot(aes(x = time, y = Shannon, color = treatment, group = time))  + facet_wrap("treatment", nrow = 2)

ggplot(alphadiv) + geom_boxplot(aes(x = time, y = Simpson, color = treatment, group = time)) + facet_wrap("treatment", nrow = 2)




ggplot(alphadiv) + geom_boxplot(aes(x = time, y = Shannon, fill = treatment)) + ylim(4, 6) + theme_bw()
ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/ShannonDiv.eps", device = "eps", width = 8, height = 6)
ggplot(alphadiv) + geom_boxplot(aes(x = time, y = Observed, fill = treatment)) + theme_bw()
ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/ObservedASVs.eps", device = "eps", width = 8, height = 6)

#ggplot(alphadiv) + geom_boxplot(aes(x = treatment, y = Shannon, fill = time))+ ylim(4, 6)
```
...
com_ord <- ordinate(ps16S_bac_rarefy_iter_noT0, method = "NMDS", distance = "bray", k = 3)
ord_plot<-plot_ordination(ps16S_bac_rarefy_iter_noT0, ordination = com_ord, type = "samples", axes = 1:2, color = "Treatment", shape = "Timepoint")
...
com_df_nmds$cumEh <- sample_data(ps16S_bac_rarefy_iter_noT0)$CumulativeEh
com_df_nmds$avgEh <- sample_data(ps16S_bac_rarefy_iter_noT0)$AverageEh
com_df_nmds$pH <- sample_data(ps16S_bac_rarefy_iter_noT0)$pH
com_df_nmds$EC <- sample_data(ps16S_bac_rarefy_iter_noT0)$EC
com_df_nmds$NitrateN <- sample_data(ps16S_bac_rarefy_iter_noT0)$NitrateN
com_df_nmds$timepoint <- sample_data(ps16S_bac_rarefy_iter_noT0)$Timepoint
com_df_nmds$treatment<- sample_data(ps16S_bac_rarefy_iter_noT0)$Treatment
com_df_nmds$covercrop <- sample_data(ps16S_bac_rarefy_iter_noT0)$Covercrop
com_df_nmds$wheatm <- sample_data(ps16S_bac_rarefy_iter_noT0)$WheatM
com_df_nmds$time <- sample_data(ps16S_bac_rarefy_iter_noT0)$time

#plot nmds 
plot(com_df_nmds$MDS1 ~ com_df_nmds$pH)

#no clear trends in ordination axes along Eh
plot(com_df_nmds$MDS1 ~ com_df_nmds$cumEh)
plot(com_df_nmds$MDS2 ~ com_df_nmds$cumEh)

com_df_nmds$treatment <- factor(com_df_nmds$treatment, levels = c("F+W", "F+WM", "CC", "CC+WM", "Tr", "Tr+WM", "2spp", "2spp+WM"), ordered = T)

ggplot(com_df_nmds, aes(x = MDS1, y = pH, color = treatment, shape = timepoint, alpha = pH)) + 
  geom_point(size = 7) +  
  geom_smooth(method = "lm", se = FALSE, aes(group = 1)) +  # Add trend line for all data
  theme_classic() + 
  scale_shape_manual(values = 1:nlevels(com_df_nmds$timepoint)) +
  labs(x = "MDS1", y = "pH", alpha = "pH", color = "Treatment", shape = "Timepoint") +
  ggtitle("Regression of pH vs. MDS1")  # Add title


ggplot(com_df_nmds, aes(x = MDS1, y = pH)) + 
  geom_point(size = 7) +  
  geom_smooth(method = "lm", se = FALSE, aes(group = 1)) +  # Add trend line for all data
  theme_classic() +
  labs(x = "MDS1", y = "pH", alpha = "pH", color = "Treatment", shape = "Timepoint") +
...
#ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/ph_X_MDS1.eps", device = "eps", width = 8, height = 6)

```

```{r}

#ggplot with com_df_nmds

centroids_tp <- aggregate(cbind(MDS1,MDS2)~timepoint, com_df_nmds, mean)
centroids_trt <- aggregate(cbind(MDS1,MDS2)~treatment, com_df_nmds, mean)
centroids_trt_tp <- aggregate(cbind(MDS1,MDS2)~treatment*timepoint, com_df_nmds, mean)
##Choose either 95% CI or SE
#f <- function(z) qt(0.025,df=length(z)-1, lower.tail=F)* sd(z)/sqrt(length(z) #SE
f <- function(z) qt(0.025,df=length(z)-1, lower.tail=F)* sd(z)/sqrt(length(z)) #calculate 95% CI
se_trt        <- aggregate(cbind(se.x=MDS1,se.y=MDS2) ~ treatment, com_df_nmds,f)
se_trt_tp        <- aggregate(cbind(se.x=MDS1,se.y=MDS2) ~ treatment*timepoint, com_df_nmds,f)
centroids_trt <- merge(centroids_trt, se_trt, by="treatment") 


centroids_trt_tp <- merge(centroids_trt_tp, se_trt_tp) 


#supp figure 4
#TRT AND TP
ggplot(com_df_nmds, aes(MDS1, MDS2, color = as.factor(treatment), shape = timepoint)) +
  geom_point(size=1) + geom_point(data=centroids_trt_tp,size=7, aes(shape = timepoint))+ 
  scale_shape_manual(values = 1:nlevels(centroids_trt_tp$timepoint)) + theme_bw() + ylim(-1,1)

ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/Treatment_Timepoint_Ord.eps", device = "eps", width = 8, height = 6)

#TRT
ggplot(com_df_nmds, aes(MDS1,MDS2, color = as.factor(treatment))) +
  geom_point(size=1) + geom_point(data=centroids_trt,size=7) + theme_bw() + ylim(-1,1) +
  geom_errorbar(data=centroids_trt,aes(ymin=MDS2-se.y,ymax=MDS2+se.y),width=0.1)+
  geom_errorbarh(data=centroids_trt,aes(xmin=MDS1-se.x,xmax=MDS1+se.x),height=0.1)

#ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/Treatment_Ord.eps", device = "eps", width = 8, height = 6)
...
ord_plot + theme_bw() + 
  scale_shape_manual(values = 1:nlevels(sample_data(ps16S_bac_rarefy_iter)$Timepoint)) 


#axis 1 with cum EH
summary(lm(com_df_nmds$MDS1 ~ com_df_nmds$cumEh))
hist(residuals(lm(com_df_nmds$MDS1 ~ com_df_nmds$cumEh)))
#looks normal enough, but just in case heres a non-parametric regression
summary(lm(MDS1 ~ cumEh, data = com_df_nmds))
#summary(lm(com_df_nmds$MDS1 ~ com_df_nmds$pH, dataframe = com_df_nmds))

#axis 2 with cum EH
summary(lm(com_df_nmds$MDS2 ~ com_df_nmds$cumEh))
hist(residuals(lm(com_df_nmds$MDS2 ~ com_df_nmds$cumEh)))
#looks normal enough, but just in case heres a non-parametric regression
summary(lm(MDS2 ~ cumEh, data = com_df_nmds))
#summary(lm(com_df_nmds$MDS1 ~ com_df_nmds$pH, dataframe = com_df_nmds))



#axis 1 with avg eh
summary(lm(com_df_nmds$MDS1 ~ com_df_nmds$avgEh))
hist(residuals(lm(com_df_nmds$MDS1 ~ com_df_nmds$avgEh)))
#looks normal enough, but just in case heres a non-parametric regression
summary(lm(MDS1 ~ avgEh, data = com_df_nmds))
#summary(lm(com_df_nmds$MDS1 ~ com_df_nmds$pH, dataframe = com_df_nmds))

#axis 2 with avg eh
summary(lm(com_df_nmds$MDS2 ~ com_df_nmds$avgEh))
hist(residuals(lm(com_df_nmds$MDS2 ~ com_df_nmds$avgEh)))
#looks normal enough, but just in case heres a non-parametric regression
summary(lm(MDS2 ~ avgEh, data = com_df_nmds))
#summary(lm(com_df_nmds$MDS1 ~ com_df_nmds$pH, dataframe = com_df_nmds))
...
#EC
ggplot(com_df_nmds, aes(x = MDS1, y = EC)) + 
  geom_point(size = 7) +  
  geom_smooth(method = "lm", se = FALSE, aes(group = 1)) +  # Add trend line for all data
  theme_classic() +
  labs(x = "MDS1", y = "EC", alpha = "EC", color = "Treatment", shape = "Timepoint") +
  ggtitle("Regression of EC vs. MDS1") + ylim(0, 0.5) # Add title


ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/EC_X_MDS1.eps", device = "eps", width = 8, height = 6)

#axis 1 with pH
summary(lm(com_df_nmds$MDS1 ~ com_df_nmds$EC))
hist(residuals(lm(com_df_nmds$MDS1 ~ com_df_nmds$EC)))
#looks normal enough, but just in case heres a non-parametric regression
summary(lm(MDS1 ~ EC, data = com_df_nmds))
#summary(lm(com_df_nmds$MDS1 ~ com_df_nmds$pH, dataframe = com_df_nmds))
```
...
ps_adonis<-function(physeq){
  ...
  vegan::adonis2(physeq_dist ~  Covercrop + WheatM + Timepoint + CumulativeEh + AverageEh + pH + EC + NitrateNperkgDrySoil + NH4NperkgDrySoil , data = md_tab, permutations = 100)
   vegan::adonis2(physeq_dist ~  Covercrop + WheatM + Timepoint +  pH + EC + CumulativeEh + AverageEh + NitrateNperkgDrySoil + NH4NperkgDrySoil , data = md_tab, permutations = 100)
}
ps_adonis(ps16S_bac_rarefy_iter_noT0)
...
dist_bt_groups_filtered_2_matched$Treatment<-str_split(dist_bt_groups_filtered_2_matched$Trt2, "[[+]]", simplify = T)[,1]
dist_bt_groups_filtered_2_matched$WheatM<-str_split(dist_bt_groups_filtered_2_matched$Trt2, "[[+]]", simplify = T)[,2]
dist_bt_groups_filtered_2_matched$WheatM <- ifelse(dist_bt_groups_filtered_2_matched$WheatM == "W" | dist_bt_groups_filtered_2_matched$WheatM == "", "NOWM", dist_bt_groups_filtered_2_matched$WheatM)


mod_distbtpts<-aov(Distance ~ Treatment * WheatM + Label, data = dist_bt_groups_filtered_2_matched)
#no difference
summary(mod_distbtpts)
hist(mod_distbtpts$residuals)
TukeyHSD(mod_distbtpts, "Treatment")
plot(TukeyHSD(mod_distbtpts, "Treatment"))

TukeyHSD(mod_distbtpts, "WheatM")
plot(TukeyHSD(mod_distbtpts, "WheatM"))


dist_bt_groups_filtered_2_matched$Trt1 <- factor(dist_bt_groups_filtered_2_matched$Trt1, levels = c("F+W", "F+WM", "CC", "CC+WM", "Tr", "Tr+WM", "2spp", "2spp+WM"), ordered = T)
...
ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/suppFigure9.eps", device = "eps", width = 10, height = 8)

#Supp figure 8
ggplot(data = dist_bt_groups_filtered_2_matched) + geom_boxplot(mapping = aes(x = Trt1, y = Distance, fill = Trt1)) + facet_wrap(~Label) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))  
ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/suppFigure8.eps", device = "eps", width = 10, height = 8)

```

```{r}
#t3 to t4 comparison 
dist_bt_groups_filtered_4 <- dist_bt_groups_filtered %>% filter(Label == "Between t3 and t4")


#MATCH FIRST SAMPLE TO METADAT TO EXTRACT TREATMENT
dist_bt_groups_filtered_4$Trt1 <- 'NA'
for (i in 1:nrow(dist_bt_groups_filtered_4)){
  for(j in 1:nrow(md_tab)){
      if(dist_bt_groups_filtered_4[i,]$Item1==md_tab[j,]$sample.names){
        dist_bt_groups_filtered_4[i,]$Trt1 <- as.character(md_tab[j,]$Treatment)
      }
    else{}
  }
}
...
#MATCH SECOND SAMPLE TO METADAT TO EXTRACT TREATMENT
dist_bt_groups_filtered_4$Trt2 <- 'NA'
for (i in 1:nrow(dist_bt_groups_filtered_4)){
  for(j in 1:nrow(md_tab)){
      if(dist_bt_groups_filtered_4[i,]$Item2==md_tab[j,]$sample.names){
        dist_bt_groups_filtered_4[i,]$Trt2 <- as.character(md_tab[j,]$Treatment)
      }
    else{}
  }
}
...
#FILTER TO ONLY INCLUDE COMPARISONS WITHIN A TREATMENT GROUP
dist_bt_groups_filtered_4_matched <- dist_bt_groups_filtered_4 %>% filter(Trt1 == Trt2)
dist_bt_groups_filtered_4_matched$Treatment<-str_split(dist_bt_groups_filtered_4_matched$Trt2, "[[+]]", simplify = T)[,1]
dist_bt_groups_filtered_4_matched$WheatM<-str_split(dist_bt_groups_filtered_4_matched$Trt2, "[[+]]", simplify = T)[,2]
dist_bt_groups_filtered_4_matched$WheatM <- ifelse(dist_bt_groups_filtered_4_matched$WheatM == "W" | dist_bt_groups_filtered_4_matched$WheatM == "", "NOWM", dist_bt_groups_filtered_4_matched$WheatM)

mod_distt3_t4<-aov(Distance ~ Treatment * WheatM, data = dist_bt_groups_filtered_4_matched)
#no difference
summary(mod_distt3_t4)
TukeyHSD(mod_distt3_t4, "Treatment")
TukeyHSD(mod_distt3_t4, "WheatM")

ggplot(data = dist_bt_groups_filtered_4_matched) + geom_boxplot(mapping = aes(x = Label, y = Distance, fill = Trt1)) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))  
...
dist_bt_groups_filtered_3_matched$Treatment<-str_split(dist_bt_groups_filtered_3_matched$Trt2, "[[+]]", simplify = T)[,1]
dist_bt_groups_filtered_3_matched$WheatM<-str_split(dist_bt_groups_filtered_3_matched$Trt2, "[[+]]", simplify = T)[,2]
dist_bt_groups_filtered_3_matched$WheatM <- ifelse(dist_bt_groups_filtered_3_matched$WheatM == "W" | dist_bt_groups_filtered_3_matched$WheatM == "", "NOWM", dist_bt_groups_filtered_3_matched$WheatM)

mod_distto_t6<-aov(Distance ~ Treatment * WheatM, data = dist_bt_groups_filtered_3_matched)
#no difference
summary(mod_distto_t6)
plot(TukeyHSD(mod_distto_t6, "Treatment"))
plot(TukeyHSD(mod_distto_t6, "WheatM"))

dist_bt_groups_filtered_3_matched$Trt1 <- factor(dist_bt_groups_filtered_3_matched$Trt1, levels = c("F+W", "F+WM", "CC", "CC+WM", "Tr", "Tr+WM", "2spp", "2spp+WM"), ordered = T)
#supp figure 10

ggplot(data = dist_bt_groups_filtered_3_matched) + geom_boxplot(mapping = aes(x = Label, y = Distance, fill = Trt1)) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))  

ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/suppFigure10.eps", device = "eps", width = 10, height = 8)
...
```{r}
sample_data(ps16S_bac_rarefy_iter)$tp_trt <- paste(sample_data(ps16S_bac_rarefy_iter)$Timepoint, sample_data(ps16S_bac_rarefy_iter)$Treatment, sep = "_")

SD <- merge_samples(ps16S_bac_rarefy_iter, "trt_tp")
sd <- transform_sample_counts(SD, function(x) x / sum(x))
sample_data(sd)$trt_tp <- rownames(sample_data(sd))

phyloseq::plot_bar(sd, x = "trt_tp", fill = "Class") + geom_bar(stat="identity") + scale_fill_manual(values = col_vector)


sample_data(ps16S_bac_rarefy_iter)$trt_tp <- paste( sample_data(ps16S_bac_rarefy_iter)$Treatment, sample_data(ps16S_bac_rarefy_iter)$Timepoint, sep = "_")

SD <- merge_samples(ps16S_bac_rarefy_iter, "trt_tp")
sd <- transform_sample_counts(SD, function(x) x / sum(x))
sample_data(sd)$trt_tp <- rownames(sample_data(sd))

phyloseq::plot_bar(sd, x = "tp_trt", fill = "Class") + geom_bar(stat="identity") + scale_fill_manual(values = col_vector)
...
#baruta_taxa<-baruta_phyloseq(ps16S_bac_rarefy_iter, GroupingVar = "Timepoint")

#ps_baruta_time <- subset_taxa(ps16S_bac_rarefy_iter, taxa_names(ps16S_bac_rarefy_iter) %in% baruta_taxa)

#phyloseq::plot_bar(ps_baruta_time, x = "trt_tp", fill = "Order") + geom_bar(stat="identity") + scale_fill_manual(values = col_vector)
#phyloseq::plot_bar(ps_baruta_time, x = "tp_trt", fill = "Order") + geom_bar(stat="identity") + scale_fill_manual(values = col_vector)


#ps_baruta_time_merged <- merge_samples(ps_baruta_time, "tp_trt")
#sample_data(ps_baruta_time_merged)$tp_trt <- rownames(sample_data(ps_baruta_time_merged))
#plot_heatmap(ps_baruta_time_merged, sample.label="tp_trt", taxa.label = "Family")

#ps_baruta_time_merged <- merge_samples(ps_baruta_time, "trt_tp")
#sample_data(ps_baruta_time_merged)$trt_tp <- rownames(sample_data(ps_baruta_time_merged))
#plot_heatmap(ps_baruta_time_merged, sample.label="trt_tp", taxa.label = "Family")


###GENUS
ps_genus<-tax_glom(ps16S_bac_rarefy_iter,taxrank = "Genus")
#ps_genus <- subset_samples(ps_genus, Timepoint != "t0" & Timepoint != "t6")
table(sample_data(ps_genus)$Timepoint)
baruta_taxa_genus<-baruta_phyloseq(ps_genus, GroupingVar = "Timepoint")
ps_baruta_time_genus <- subset_taxa(ps_genus, taxa_names(ps_genus) %in% baruta_taxa_genus)

#phyloseq::plot_bar(ps_baruta_time_genus, x = "trt_tp", fill = "Order") + geom_bar(stat="identity") + scale_fill_manual(values = col_vector)

#phyloseq::plot_bar(ps_baruta_time_genus, x = "tp_trt", fill = "Order") + geom_bar(stat="identity") + scale_fill_manual(values = col_vector)


ps_baruta_time_merged_genus <- merge_samples(ps_baruta_time_genus, "tp_trt")

sample_data(ps_baruta_time_merged_genus)$tp_trt <- rownames(sample_data(ps_baruta_time_merged_genus))

#sample_data(ps_baruta_time_merged_genus)$trt_tp <- rownames(sample_data(ps_baruta_time_merged_genus))

#phyloseq::plot_heatmap(ps_baruta_time_merged_genus, sample.label="tp_trt", taxa.label = "Genus", sample.order = "tp_trt")+ theme (axis.text.y = element_text(size=12))+ theme (axis.text.x = element_text(size=12))


map<-phyloseq::plot_heatmap(ps_baruta_time_merged_genus, sample.label="tp_trt", taxa.label = "Family", sample.order = "trt_tp", high = "red", low = "blue") + theme (axis.text.y = element_text(size=12))+ theme (axis.text.x = element_text(size=12)) 
y_order <- ggplot_build(map)$layout$panel_params[[1]]$y.sec$breaks

#phyloseq::plot_heatmap(ps_baruta_time_merged_genus, sample.label="tp_trt", taxa.label = "Phylum", sample.order = "tp_trt") + theme (axis.text.y = element_text(size=12))+ theme (axis.text.x = element_text(size=12))


#ps_baruta_time_merged_genus <- merge_samples(ps_baruta_time, "trt_tp")
#sample_data(ps_baruta_time_merged_genus)$trt_tp <- rownames(sample_data(ps_baruta_time_merged_genus))
#phyloseq::plot_heatmap(ps_baruta_time_merged_genus, sample.label="trt_tp", taxa.label = "Genus")
...
zscore heatmap by timepoint indicators
```{r}
ps_genus<-tax_glom(ps16S_bac_rarefy_iter,taxrank = "Genus")
#ps_genus <- subset_samples(ps_genus, Timepoint != "t0" & Timepoint != "t6")
table(sample_data(ps_genus)$Timepoint)
...
df_counts<-otu_table(ps_baruta_time_merged_genus)
colSums(df_counts)


filt_zscore_ps <- filter_taxa(ps_baruta_time_merged_genus, function(x) sum(x) > 1000, TRUE)

z_ps_baruta_time_merged_genus <- microbiome::transform(filt_zscore_ps, transform = "Z", target = "OTU")

tax_z_ps_baruta_time_merged_genus<-data.frame(tax_table(z_ps_baruta_time_merged_genus))
tax_z_ps_baruta_time_merged_genus <- tax_z_ps_baruta_time_merged_genus[match(y_order, rownames(tax_z_ps_baruta_time_merged_genus)), ]
write.csv(tax_z_ps_baruta_time_merged_genus, "/Users/gordoncuster/Desktop/Git_projects/ASD/Results/timepoint_indicators_taxonomy_ordered.csv")


reversed_df <- tax_z_ps_baruta_time_merged_genus[nrow(tax_z_ps_baruta_time_merged_genus):1, ]
write.csv(reversed_df, "/Users/gordoncuster/Desktop/Git_projects/ASD/Results/timepoint_indicators_taxonomy_ordered_flipped.csv")

otu_df <- data.frame(otu_table(z_ps_baruta_time_merged_genus))
otu_df$trt_tp <- rownames(sample_data(z_ps_baruta_time_merged_genus))
otu_df$tp <- str_split(otu_df$trt_tp, pattern = "_", simplify = T)[,1]
otu_df$trt <- str_split(otu_df$trt_tp, pattern = "_", simplify = T)[,2]
otu_df$tp_trt <- paste(otu_df$tp, otu_df$trt, sep = "_")

data_long <- gather(otu_df, key = "ASV", value = "value", - tp_trt)
data_long2<-data_long
data_long2$ASV <- factor(data_long2$ASV, levels = y_order)
data_long2$value<-as.numeric(data_long2$value)

str(data_long2)

colors <- c("deepskyblue3", "bisque", "coral3")

library(viridis)
ggplot(data_long2, aes(x = tp_trt, y = ASV , fill = value)) +
  geom_tile() +
  #scale_fill_viridis(option = "plasma") +
scale_fill_gradientn(colors = colors)+
  labs(title = "Heatmap of Data",
       x = "Treatment",
       y = "ASV")+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

#ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/Timepoint_indicators_heatmap.eps", device = "eps", width = 8, height = 6)
```




zscore heatmap by treatment indicators - no t0 or t6
```{r}
ps_genus<-tax_glom(ps16S_bac_rarefy_iter,taxrank = "Genus")
#ps_genus <- subset_samples(ps_genus, Timepoint != "t0" & Timepoint != "t6")
table(sample_data(ps_genus)$Timepoint)
baruta_taxa_genus<-baruta_phyloseq(ps_genus, GroupingVar = "Treatment")
ps_baruta_time_genus <- subset_taxa(ps_genus, taxa_names(ps_genus) %in% baruta_taxa_genus)


ps_baruta_time_merged_genus <- merge_samples(ps_baruta_time_genus, "trt_tp")
...
map<-phyloseq::plot_heatmap(ps_baruta_time_merged_genus, sample.label="trt_tp", taxa.label = "Family", sample.order = "trt_tp", high = "red", low = "blue") + theme (axis.text.y = element_text(size=12))+ theme (axis.text.x = element_text(size=12)) 
y_order <- ggplot_build(map)$layout$panel_params[[1]]$y.sec$breaks
...
tax_z_ps_baruta_time_merged_genus<-data.frame(tax_table(z_ps_baruta_time_merged_genus))
tax_z_ps_baruta_time_merged_genus <- tax_z_ps_baruta_time_merged_genus[match(y_order, rownames(tax_z_ps_baruta_time_merged_genus)), ]
write.csv(tax_z_ps_baruta_time_merged_genus, "/Users/gordoncuster/Desktop/Git_projects/ASD/Results/treatment_indicators_taxonomy_ordered.csv")

reversed_df <- tax_z_ps_baruta_time_merged_genus[nrow(tax_z_ps_baruta_time_merged_genus):1, ]
write.csv(reversed_df, "/Users/gordoncuster/Desktop/Git_projects/ASD/Results/treatment_indicators_taxonomy_ordered_flipped.csv")

otu_df <- data.frame(otu_table(z_ps_baruta_time_merged_genus))
otu_df$trt_tp <- rownames(sample_data(z_ps_baruta_time_merged_genus))
otu_df$tp <- str_split(otu_df$trt_tp, pattern = "_", simplify = T)[,2]
otu_df$trt <- str_split(otu_df$trt_tp, pattern = "_", simplify = T)[,2]
otu_df$tp_trt <- paste(otu_df$tp, otu_df$trt, sep = "_")

data_long <- gather(otu_df, key = "ASV", value = "value", - trt_tp)
data_long2<-data_long
data_long2$ASV <- factor(data_long2$ASV, levels = y_order)
data_long2$value<-as.numeric(data_long2$value)
...
library(viridis)
ggplot(data_long2, aes(x = trt_tp, y = ASV , fill = value)) +
  geom_tile() +
  #scale_fill_viridis(option = "plasma") +
scale_fill_gradientn(colors = colors)+
  labs(title = "Heatmap of Data",
...
#ggsave("/Users/gordoncuster/Desktop/Git_projects/ASD/Results/Images/Treatment_indicators_heatmap.eps", device = "eps", width = 8, height = 6)
```



Boxplot of important Phyla - Firmicutes - Proteobacteria - Acidiobacteria

```{r}
#Create new column of phases
sample_data(ps16S_bac_rarefy_iter)$phase <- as.factor(sample_data(ps16S_bac_rarefy_iter)$Timepoint)

sample_data(ps16S_bac_rarefy_iter) <- data.frame(sample_data(ps16S_bac_rarefy_iter)) %>%
  mutate(phase = case_when(
    Timepoint == "t0" ~ "Phase I",
    Timepoint %in% c("t1", "t2", "t3", "t4", "t5") ~ "Phase II",
    Timepoint == "t6" ~ "Phase III"
  ))

sample_data(ps16S_bac_rarefy_iter)$phase <- as.factor(sample_data(ps16S_bac_rarefy_iter)$phase)

physeq_phylum <- tax_glom(ps16S_bac_rarefy_iter, taxrank = "Phylum")

#create phyla specific phylsoeq ojbects
firmi <- subset_taxa(physeq_phylum, Phylum == "Firmicutes")
proteo <- subset_taxa(physeq_phylum, Phylum == "Proteobacteria")
acidio <- subset_taxa(physeq_phylum, Phylum == "Acidobacteria")


# Melt phyloseq object to a data frame
physeq_phylum_melted <- psmelt(firmi)

# Create the boxplot
ggplot(physeq_phylum_melted, aes(x = Phase, y = Abundance, fill = Phylum)) +
  geom_boxplot() +
  facet_wrap(~ Phylum, scales = "free") +  # Facet by Phylum
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Phylum-level Taxa Abundance by Phase",
       x = "Phase",
       y = "Abundance")  +theme_bw()



physeq_phylum_melted <- psmelt(proteo)
ggplot(physeq_phylum_melted, aes(x = Phase, y = Abundance, fill = Phylum)) +
  geom_boxplot() +
  facet_wrap(~ Phylum, scales = "free") +  # Facet by Phylum
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
physeq_phylum_melted <- psmelt(acidio)
ggplot(physeq_phylum_melted, aes(x = Phase, y = Abundance, fill = Phylum)) +
  geom_boxplot() +
  facet_wrap(~ Phylum, scales = "free") +  # Facet by Phylum
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
```{r}
physeq_phylum <- tax_glom(ps16S_bac_rarefy_iter, taxrank = "Class")

#create phyla specific phylsoeq ojbects
bacilli <- subset_taxa(physeq_phylum, Class == "Bacilli")
bacilli <- subset_taxa(physeq_phylum, Class == "Bacilli")

alphap <- subset_taxa(physeq_phylum, Class == "Alphaproteobacteria")
deltap <- subset_taxa(physeq_phylum, Class == "Bacilli")
gammap <- subset_taxa(physeq_phylum, Class == "Gammaproteobacteria")

bacilli <- subset_taxa(physeq_phylum, Class == "Bacilli")
bacilli <- subset_taxa(physeq_phylum, Class == "Bacilli")

# Melt phyloseq object to a data frame
physeq_phylum_melted <- psmelt(bacilli)

# Create the boxplot
ggplot(physeq_phylum_melted, aes(x = Phase, y = Abundance, fill = Class)) +
  geom_boxplot() +
  facet_wrap(~ Class, scales = "free") +  # Facet by Phylum
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Phylum-level Taxa Abundance by Phase",
       x = "Phase",
       y = "Abundance")  +theme_bw()
